...
public class AddArticle  {
	...
	public void initAdd() throws Exception {
		...
		Label lb=new Label("Ajouter Article ");
		...
		bt1.setOnMouseClicked((MouseEvent even)-> {
			...
			try {
				...
				err_up.setText("les champs non remplis ou mal remplis sont pris pour 0");
			}
			Article a=null;
			...
			Data d=new Data();
			if(d.ajout(a,type)) {
				err.setText("Article ajouté");
			}else {
				err_up.setText("un erreur s'est produit.Veuillez reessayer");
				err.setText("");
			}
		});
		...
		VBox v = new VBox();
		v.setPadding(new Insets(10, 25, 25, 25));
		v.setSpacing(13.0);
		v.getChildren().addAll(er_up,er,lb1,tf1, lb2,tf2, lb3,tf3, lb4,tf4, lb5,tf5, lb6,tf6, h);
		v.setLayoutY(100);
		v.setLayoutX(300);
